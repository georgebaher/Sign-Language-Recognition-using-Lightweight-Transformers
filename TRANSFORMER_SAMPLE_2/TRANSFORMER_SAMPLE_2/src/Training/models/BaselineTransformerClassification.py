...
class BaselineTransformerClassification(nn.Module):
    ...
    def __init__(self, num_classes, hidden_dim=55, n_heads = 9, max_seq_len = 50):
        ...
        self.output_pos_encoding = PositionalEncodingSinCos(hidden_dim, 0.0)
    ...
    def forward(self, inputs):
        h = (inputs).float() # torch.unsqueeze(inputs.flatten(start_dim=1), 1).float()
...
class BaselineTransformerClassificationNoPE(nn.Module):
    """
    Implementation of the SPOTER (Sign POse-based TransformER) architecture for sign language recognition from sequence
    of skeletal data.
    """
    ...
    def forward(self, inputs):
        henc = (inputs).float() # torch.unsqueeze(inputs.flatten(start_dim=1), 1).float()
        #Apply positional emb.
        #henc = self.output_pos_encoding(h)
