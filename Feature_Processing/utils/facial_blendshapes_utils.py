...
def compute_video_facial_blendshapes(video_path: str, gloss: str, show_landmarks: bool = False):
    ...
    :return: DataFrame of facial blendshapes with one row per frame (padded with -2 where missing), optional video path, and failed to extract landmarks flag
    """
    ...
    # -------------------------
    if not frames_data:
        print(f"⚠️ No facial landmarks detected at all in video {video_id}")
        padded_data = []
