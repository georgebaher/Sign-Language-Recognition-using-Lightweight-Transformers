...
from WLASL_feature_processing.models.hand_model import HandModel
...
def compute_hand_angles(df: pd.DataFrame, video_id: str, gloss: str) -> pd.DataFrame:
    """
       ...
       :param df: DataFrame containing all videos hands avasag_vitpose_extracted_landmarks with columns like 'video_id, gloss, LH#0_x', ..., 'RH#20_z'
    ...
    for _, row in vid_df.iterrows():
        try:
            lh = row[lh_cols].values.astype(float).reshape(-1, 2)   # 3)
            rh = row[rh_cols].values.astype(float).reshape(-1, 2)   # 3)
...
def get_hand_header() -> list:
    dummy = HandModel([i for i in range(42)])     # 63)])
