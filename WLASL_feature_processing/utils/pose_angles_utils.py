...
load_dotenv()
from WLASL_feature_processing.models.pose_model import PoseModel
...
def compute_pose_angles(df: pd.DataFrame, video_id: str, gloss: str) -> pd.DataFrame:
    ...
    for _, row in vid_df.iterrows():
        try:
            pose = row[pose_cols].values.astype(float).reshape(-1, 2)  # 3)
...
def get_pose_header() -> list:
    dummy = PoseModel([i for i in range(66)])  # 99)])
...
if __name__ == "__main__":
    # TESTING ...
    all_vids_hand_pose_landmarks_df = pd.read_parquet(os.getenv("WLASL100_HAND_POSE_LANDMARKS_PATH"))
    test_video_id = "69302"

    # Test pose angles calculation
    vid_pose_angles_df = compute_pose_angles(all_vids_hand_pose_landmarks_df, test_video_id, "drink")
