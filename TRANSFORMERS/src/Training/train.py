...
def get_default_args():
    ...
    parser.add_argument("--top_features", type=str,
                        ...
                        help='Features used. Choices include body parts or "FACIAL_BLENDSHAPES" for facial top_features only.')
    parser.add_argument("--include_blendshapes", type=int, default=0,
                        help='choose whether to include facial blendshapes (with body top_features)')
    parser.add_argument("--fs", type=int,
                        default=0,
                        help='Use feature selection (only for body top_features)')
    parser.add_argument("--feature_padding_mode", type=str,
                        default="sentinel",
                        choices=["sentinel", "repeat", "truncate"],
                        help='Truncate embedding dimension (number of top_features) to be divisible by the number of heads, or pad with -2s or repeat columns')
...
def train(args):
    ...
    if dataset_name == "WLASL100":
        ...
        else:  # Original logic for body top_features
            print("[INFO] Processing WLASL100 dataset with body top_features...")
            feature_parquet_map = {
    ...
    if model2use == 'baseline_transformer':
        ...
        model = LSTMClassifier(
            input_dim=hidden_dim,  # assuming your input top_features == hidden_dim
            hidden_dim=hidden_dim,
            num_classes=num_classes,
            num_layers=n_layers,
            n_heads=n_heads
